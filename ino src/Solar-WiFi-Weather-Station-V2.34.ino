...
//**************************Sending Data to Heroku and ThingSpeak*********************************
  // code block for uploading data to Heroku App
  
  if (App1 == "Heroku") {       // for posting data to Heroku
    HTTPClient http;
    http.begin("http://weather-station-server.herokuapp.com/reallysecurequoteapikeyquotezavsichkimislqcheshterabotikolkotopodulugpodobredaneslagatfalshividannirusnacitenp");
    http.addHeader("Content-Type", "application/json");
    String whole = "{\"data\":{\n \"weatherStation\":";
    whole+= weatherstationid;
    whole+= ",\n\"temperatures\":";
    whole+= adjusted_temp;
    whole+=",\n  \"humidity\":";
    whole+= adjusted_humi;
    ...
    whole+= measured_pres;
    whole+=",\n \"relativePressure\" :";
    whole+= rel_pressure_rounded;
    whole+=",\n  \"voltageBattery\":";
    whole+= volt;
    whole+=",\n  \"dewPoint\":";
    whole+= DewpointTemperature;
    whole+=",\n  \"heatIndex\":";
    whole+= HeatIndex;
    whole+=",\n  \"zambrettisWords\":\"";
    whole+= ZambrettisWords;
    whole+="\",\n  \"accuracyInPercents\":";
    whole+= accuracy_in_percent;
    whole+=",\n  \"tendInWords\":\"";
    whole+= trend_in_words;
    whole+="\",\n  \"dewPointSpread\":";
    whole+= DewPointSpread;
    whole+="\n}}";
    Serial.print(whole);
    int httpResponseCode = http.POST(whole);
    ...
    http.end();
    Serial.println("Data written to Heroku ...");
  }
